...
log_path = r"C:\Users\kabish\.gemini\antigravity-ide\brain\6bb09710-7f3e-4443-b187-ad04e16ca8dc\scratch\rename_hmi_log.txt"

# 1. GVL Content with Coarse_To_Fine_Speed
...
# 2. Auto_Batching_V14 Declaration
...
# 3. Auto_Batching_V14 Implementation
code_auto_v14 = """(* Auto Batching FB V14 with Coarse_To_Fine_Speed, HMI Target Updates, and Robust Sync *)
...
(* 5. AUTO SILOS 1..6 POURING SEQUENCE WITH HMI BIDIRECTIONAL TARGET UPDATES *)
...
CASE Step OF
    0:
        ...
        FOR i := 1 TO 6 DO
            ...
            auto_bin_motor[i] := FALSE;
        END_FOR;
        (* Allow HMI target preview or preserve target when not resetting *)
    ...
    1..6:
        ...
        (* Check if Target is configured in Recipe or directly entered from HMI *)
        IF (Mat_Idx = 0 OR (Mat_Idx <= 20 AND GVL.Recipe_Weights[Mat_Idx] = 0.0)) AND GVL.Auto_Active_Target_Weight = 0.0 THEN
            ...
            END_IF;
        ELSE
            (* Read target from Recipe; if 0, accept HMI Active Target *)
...
# 4. Semi_Auto_Batching_V14 Declaration
...
# 5. Semi_Auto_Batching_V14 Implementation
code_semi_v14 = """(* Semi-Auto Batching FB V14 with Coarse_To_Fine_Speed, HMI Target Updates, and Robust Sync *)
...
# 6. PLCopen XML for batching14 with renamed Coarse_To_Fine_Speed pins
b14_xml_path = r"C:\Users\kabish\.gemini\antigravity-ide\brain\6bb09710-7f3e-4443-b187-ad04e16ca8dc\scratch\batching14_renamed.xml"
b14_xml = """<?xml version="1.0" encoding="utf-8"?>
<project xmlns="http://www.plcopen.org/xml/tc6_0200">
  <fileHeader companyName="" productName="Automation Builder 2.7 - Basic" productVersion="Automation Builder 2.7" creationDateTime="2026-08-29T13:35:00" />
      ...
      <pou name="batching14" pouType="program">
        ...
        <body>
          <FBD>
            ...
            <outVariable localId="39"><position x="650" y="280" /><connectionPointIn><connection refLocalId="1" formalParameter="Excess_Alarm" /></connectionPointIn><expression>GVL.Auto_Excess_Alarm</expression></outVariable>
            <outVariable localId="40"><position x="650" y="300" /><connectionPointIn><connection refLocalId="1" formalParameter="Error_Code" /></connectionPointIn><expression>Auto_Err</expression></outVariable>
            <outVariable localId="41"><position x="650" y="320" /><connectionPointIn><connection refLocalId="1" formalParameter="Status_Message" /></connectionPointIn><expression>Auto_Msg</expression></outVariable>
            ...
            <outVariable localId="79"><position x="650" y="680" /><connectionPointIn><connection refLocalId="2" formalParameter="Excess_Alarm" /></connectionPointIn><expression>GVL.Semi_Auto_Excess_Alarm</expression></outVariable>
            <outVariable localId="80"><position x="650" y="700" /><connectionPointIn><connection refLocalId="2" formalParameter="Error_Code" /></connectionPointIn><expression>Semi_Auto_Err</expression></outVariable>
            <outVariable localId="81"><position x="650" y="720" /><connectionPointIn><connection refLocalId="2" formalParameter="Status_Message" /></connectionPointIn><expression>Semi_Auto_Msg</expression></outVariable>
          </FBD>
        </body>
      </pou>
    </pous>
...
with open(b14_xml_path, "w") as f:
    f.write(b14_xml)

with open(log_path, "w") as f:
    f.write("Applying Coarse_To_Fine_Speed renaming and HMI target weight updates...\n")
    try:
        ...
        f.write("GVL updated with Coarse_To_Fine_Speed.\n")
        
        # 2. Update Auto_Batching_V14
        p = proj.find("Auto_Batching_V14", True)[0]
        p.textual_declaration.replace(decl_auto_v14)
        p.textual_implementation.replace(code_auto_v14)
        f.write("Auto_Batching_V14 updated.\n")
        
        # 3. Update Semi_Auto_Batching_V14
        p = proj.find("Semi_Auto_Batching_V14", True)[0]
        p.textual_declaration.replace(decl_semi_v14)
        p.textual_implementation.replace(code_semi_v14)
        f.write("Semi_Auto_Batching_V14 updated.\n")
        
        # 4. Re-import batching14 with updated FBD pins
        p_b14 = proj.find("batching14", True)
        for b in p_b14:
            b.remove()
        app.import_xml(b14_xml_path)
        f.write("batching14 FBD re-imported with renamed pins.\n")
        
        proj.save()
        proj.close()
        f.write("All project updates saved successfully.\n")
