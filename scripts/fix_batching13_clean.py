...
log_path = r"C:\Users\kabish\.gemini\antigravity-ide\brain\6bb09710-7f3e-4443-b187-ad04e16ca8dc\scratch\clean_fix_log.txt"

# 1. GVL with Auto_Bin_Cutoff_Weights, Semi_Auto_Bin_Cutoff_Weights, and V14 extensions
...
code_auto_v13 = """(* Auto Batching FB V13 with Real-Time Instant Excess Verification & Clean Multi-Cycle Reset *)
...
code_semi_v13 = """(* Semi-Auto Batching FB V13 with Real-Time Instant Excess Verification & Clean Multi-Cycle Reset *)
...
code_b13 = """(* 1. STARTUP CONFIGURATION, RECIPE TOTALS, AND WEIGHT CHECKS *)
...
with open(log_path, "w") as f:
    try:
        ...
        f.write("GVL updated with Auto_Bin_Cutoff_Weights.\n")
        
        # 2. Revert Auto_Batching_V13
        p = proj.find("Auto_Batching_V13", True)[0]
        p.textual_declaration.replace(decl_auto_v13)
        p.textual_implementation.replace(code_auto_v13)
        f.write("Auto_Batching_V13 reverted.\n")
        
        # 3. Revert Semi_Auto_Batching_V13
        p = proj.find("Semi_Auto_Batching_V13", True)[0]
        p.textual_declaration.replace(decl_semi_v13)
        p.textual_implementation.replace(code_semi_v13)
        f.write("Semi_Auto_Batching_V13 reverted.\n")
        
        # 4. Revert batching13
        p = proj.find("batching13", True)[0]
        p.textual_declaration.replace(decl_b13)
        p.textual_implementation.replace(code_b13)
        f.write("batching13 reverted.\n")
